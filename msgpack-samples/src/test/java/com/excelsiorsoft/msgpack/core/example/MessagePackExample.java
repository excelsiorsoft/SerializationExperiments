package com.excelsiorsoft.msgpack.core.example;
...
public class MessagePackExample {
	...
	public void basicUsage() throws IOException {
		...
		packer.packInt(1).packString("leo").packArrayHeader(2)
				.packString("xxx-xxxx").packString("yyy-yyyy");
		packer.close();
		...
		for (int i = 0; i < numPhones; ++i) {
			phones[i] = unpacker.unpackString(); // phones = {"xxx-xxxx",
													// "yyy-yyyy"}
		}
		unpacker.close();
		System.out.println(String.format("id:%d, name:%s, phone:[%s]", id,
				name, join(phones)));
	}
	 ...
	 */
	@Test
	public  void packer() throws IOException {
		...
		packer.packExtendedTypeHeader(1, 10); // type number [0, 127], data byte
												// length
		...
		ByteBuffer bb = ByteBuffer.wrap(new byte[] { 'b', 'i', 'n', 'a', 'r',
				'y', 'd', 'a', 't', 'a' });
	...
	public void readAndWriteFile() throws IOException {
		...
		// Write packed data to a file. No need exists to wrap the file stream
		// with BufferedOutputStream, since MessagePacker has its own buffer
		MessagePacker packer = MessagePack
				.newDefaultPacker(new FileOutputStream(tempFile));
		packer.packInt(1);
		packer.packString("Hello Message Pack!");
		packer.packArrayHeader(2).packFloat(0.1f).packDouble(0.342);
		packer.close();
		// Read packed data from a file. No need exists to wrap the file stream
		// with an buffer
		MessageUnpacker unpacker = MessagePack
				.newDefaultUnpacker(new FileInputStream(tempFile));
		while (unpacker.hasNext()) {
			// [Advanced] You can check the detailed data format with
			// getNextFormat()
			// Here is a list of message pack data format:
			// https://github.com/msgpack/msgpack/blob/master/spec.md#overview
			MessageFormat format = unpacker.getNextFormat();
			// Alternatively you can use ValueHolder to extract a value of any
			// type
			// NOTE: Value interface is in a preliminary state, so the following
			// code might change in future releases
	...
	public void configuration() throws IOException {
		...
		MessagePack.Config config = new MessagePack.ConfigBuilder()
				...
				// Drop malformed and unmappable UTF-8 characters
				.onUnmappableCharacter(CodingErrorAction.REPLACE)
				.packerBufferSize(8192 * 2).build();
